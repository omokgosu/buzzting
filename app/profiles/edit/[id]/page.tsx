...
export default function EditProfilePage() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          idealTypes: profile.idealTypes || [],
          dateStyles: profile.dateStyles || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await profileApi.update(id, {
    ...
    <div className="min-h-screen bg-[#FAF8F3]">
      <header className="bg-white/80 backdrop-blur-sm shadow-sm border-b border-[#E8DDD4] sticky top-0 z-20">
        <div className="px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link href={`/profile/${id}`} className="text-sm text-[#A08060] active:text-[#8B7355] transition-colors">
              ← 돌아가기
            </Link>
          </div>
        </div>
      ...
      <main className="px-4 py-6 pb-24">
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-[#E8DDD4]">
          <h1 className="text-xl font-bold text-[#5C4A37] mb-2">프로필 수정</h1>
